...
def adjust_scales2image(real_, scale_factor, min_size, max_size):
    init_scale_factor = scale_factor
    #opt.num_scales = int((math.log(math.pow(opt.min_size / (real_.shape[2]), 1), opt.scale_factor_init))) + 1
    num_scales = math.ceil((math.log(math.pow(min_size / (min(real_.shape[2], real_.shape[3])), 1), init_scale_factor))) + 1
    scale2stop = math.ceil(math.log(min([max_size, max([real_.shape[2], real_.shape[3]])]) / max([real_.shape[2], real_.shape[3]]), init_scale_factor))
    stop_scale = num_scales - scale2stop
    scale1 = min(max_size / max([real_.shape[2], real_.shape[3]]),1)  # min(250/max([real_.shape[0],real_.shape[1]]),1)
    real = my_imresize(real_, scale1)
    #opt.scale_factor = math.pow(opt.min_size / (real.shape[2]), 1 / (opt.stop_scale))
    scale_factor = math.pow(min_size/(min(real.shape[2],real.shape[3])),1/(stop_scale))
    scale2stop = math.ceil(math.log(min([max_size, max([real_.shape[2], real_.shape[3]])]) / max([real_.shape[2], real_.shape[3]]),init_scale_factor))
    stop_scale = num_scales - scale2stop
    return num_scales, stop_scale, scale1, scale_factor, real
...
def adjust_scales2image_SR(real_,opt):
    ...
    real = my_imresize(real_, opt.scale1, opt)
    #opt.scale_factor = math.pow(opt.min_size / (real.shape[2]), 1 / (opt.stop_scale))
...
def creat_reals_pyramid(real,reals,scale_factor, stop_scale):
    ...
    for i in range(0, stop_scale+1,1):
        ...
        curr_real = my_imresize(real,scale)
